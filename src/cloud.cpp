...
PointCart PointCyl::to_cart() const {
    double x_cart = distance * std::sin(angle * (acos(-1.0f) / 180.0f));
    double y_cart = distance * std::cos(angle * (acos(-1.0f) / 180.0f));
...
PointCart PointCyl::to_cart(float scale, float origin_x, float origin_y) const {
    double x_cart = distance * std::sin(angle * (acos(-1.0f) / 180.0f)) * scale;
    double y_cart = distance * std::cos(angle * (acos(-1.0f) / 180.0f)) * scale;
