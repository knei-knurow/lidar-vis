...
void App::print_help() {
    std::cout
        ...
        << "\t-S  --scale [scale]            scale (1mm -> 1px for scale = "
           "1.0)\n"
        << "\n"
        << "Scenarios:\n"
        << "\t0    do nothing, just grab a cloud and visualize (default)\n"
        << "\t2    save each cloud as a new screenshot (extremely "
           "unoptimized)\n"
        << "\n"
        << "\tKeyboard Shortcuts:\n"
...
bool App::init(std::vector<std::string>& args) {
    ...
    std::string scenario_val = get_flag_value(
    ...
    if (scenario_val == std::to_string(int(ScenarioType::IDLE))) {
        scenario_type = ScenarioType::IDLE;
               ...
               std::to_string(int(ScenarioType::RECORD_SERIES))) {
    ...
    if (bool(std::stringstream(get_flag_value(args, "-C", "--colormap")) >>
             colormap_temp)) {
        sfml_settings.colormap = static_cast<GUISettings::Colormap>(
            colormap_temp % GUISettings::Colormap::COLORMAP_COUNT);
    }

    if (bool(std::stringstream(get_flag_value(args, "-M", "--ptr-mode")) >>
             display_mode_temp)) {
        sfml_settings.points_display_mode =
            static_cast<GUISettings::PointsDispayMode>(
                display_mode_temp %
                GUISettings::PointsDispayMode::PTS_DISPLAY_MODE_COUNT);
    }

    if (bool(std::stringstream(get_flag_value(args, "-S", "--scale")) >>
             sfml_settings.scale)) {
    ...
    if (scenario_type == ScenarioType::RECORD_SERIES) {
        ...
        std::function<bool(void)> fn_ptr =
            std::bind(&GUI::save_screenshot, gui_.get());
