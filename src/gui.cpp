...
bool GUI::update(const Cloud& cloud) {
    ...
    if (cloud.size > 0) {
        if (settings_.autoscale) {
            ...
            if (new_scale >= settings_.scale * 1.1 ||
                new_scale <= settings_.scale * 0.9)
        ...
        render_front_line(cloud.points_cart.front().x,
                          cloud.points_cart.front().y);
...
void GUI::handle_input(const Cloud& cloud) {
    ...
    while (window_.pollEvent(event)) {
        if (event.type == sf::Event::Resized) {
            ...
            std::cout << "lidar-vis: window resized: " << event.size.width
        ...
        if (event.type == sf::Event::KeyPressed) {
            ...
            if (event.key.code == sf::Keyboard::C)
                settings_.colormap = GUISettings::Colormap(
                    (settings_.colormap + 1) % GUISettings::COLORMAP_COUNT);
            if (event.key.code == sf::Keyboard::M)
                settings_.points_display_mode = GUISettings::PointsDispayMode(
                    (settings_.points_display_mode + 1) %
...
void GUI::render_cloud_bars(const Cloud& cloud) {
    ...
    for (int j = 0; j < settings_.height; j++) {
        float dist = cloud.points_cyl[size_t(j * cloud.size / settings_.height)]
                         .distance;
        ...
        if (settings_.colormap == GUISettings::FROM_ANGLE) {
            color = calc_color_from_angle(
                float(j * cloud.size / settings_.height) / float(cloud.size));
        } else if (settings_.colormap == GUISettings::FROM_DIST) {
            color = calc_color_from_dist(
                cloud.points_cyl[size_t(j * cloud.size / settings_.height)]
                    .distance,
...
void GUI::render_cloud(const Cloud& cloud, float lightness) {
    ...
    for (int i = 0; i < cloud.size; i++) {
        ...
        if (settings_.colormap == GUISettings::FROM_ANGLE) {
            color =
                calc_color_from_angle(float(i) / float(cloud.size), lightness);
        } else if (settings_.colormap == GUISettings::FROM_DIST) {
            color = calc_color_from_dist(cloud.points_cyl[i].distance,
                                         cloud.max_distance, lightness);
        }
        render_point(cloud.points_cart[i].x, cloud.points_cart[i].y, color);
    }
}

void GUI::render_connected_cloud(const Cloud& cloud,
                                 float lightness,
                                 bool render_points) {
    if (cloud.size == 0)
        return;
    ...
    for (int i = 0; i < cloud.size; i++) {
        ...
        sf::Vertex vertex(
            sf::Vector2f(
                cloud.points_cart[i].x * settings_.scale + settings_.origin_x,
...
void GUI::render_front_line(int x, int y) {
    sf::Vertex line[] = {
        sf::Vertex(sf::Vector2f(settings_.origin_x, settings_.origin_y),
