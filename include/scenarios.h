...
/**
 * Scenario is a set of actions which are executed just after grabbing the cloud
 * data, but before its visualization by the GUI.
 */
class Scenario {
   public:
    Scenario() = default;
...
class RecordSeriesScenario : public Scenario {
   public:
    ...
    inline ScenarioType get_type() const override {
